...
import os
import sys
from datetime import datetime  # 确保导入了datetime模块
...
def main():
    ...
    try:
        while True:
            ...
            push_message("安全社区文章定时监控已启动!", f"启动时间：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}")
            ...
            time_diff = current_run_time - last_run_time
            if time_diff.seconds >= 86400:  # 86400 seconds in 24 hours
                # 重启脚本
                os.execl(sys.executable, sys.executable, *sys.argv)
                last_run_time = current_run_time  # 更新上次运行时间
